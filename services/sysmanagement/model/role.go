...
type (
	RoleModel interface {
		...
		FindOneByCode(ctx context.Context, roleCode string) (*Role, error)
	...
	Role struct {
		...
		SelectedMenuIds string         `json:"selected_menu_ids" gorm:"column:selected_menu_ids;type:varchar(2048)"` // 选择的菜单ID
...
func (m *defaultRoleModel) FindOneByCode(ctx context.Context, roleCode string) (*Role, error) {
	var result *Role
	err := m.db.Where("role_code = ?", roleCode).First(&result).Error
